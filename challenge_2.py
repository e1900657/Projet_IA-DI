...
if len(sys.argv) != 3:
    print("Usage: python challenge_2.py <path_pickle_train> <path_pickle_test>")
    sys.exit(1)
...
if True:
    ...
    resultats_train = data_train.sample(frac=0.02).reset_index(drop=True)

    if resultats_train.empty:
        print("Aucun élément avec ")
    else:
        print(len(resultats_train), "éléments ")
        ...
        predictions = ['normal' if label == 0 else 'attacker' if label == 1 else 'victim' for label in predictions]
        
        probabilities = class_probabilities.tolist() # ordre attacker,normal, victim
        for i in range(len(probabilities)):
            probabilities[i][0], probabilities[i][1] = probabilities[i][1], probabilities[i][0]
        ...
        if len(predictions) == len(class_probabilities) == len(X_test):
            ...
            output_filename = f"{res['names'][0]}_{res['names'][1]}_{res['version']}.res"
